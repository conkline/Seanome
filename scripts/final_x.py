...
if __name__ == "__main__":
    ...
    for line in open(sys.argv[1]):
        ...
        lvl1_clusts[data[1]].append([ data[0], data[2], data[3]])
    ...
    for line in open(sys.argv[2]):
        ...
        if data[0] in lvl1_clusts:
            lvl2_cigar = data[2]
            if (lvl1_clusts[data[0]])>=3: #filter by min depth
                for entry in lvl1_clusts[data[0]]:
                    lvl1_cigar = entry[1]
                    lvl1_strand = entry[2]
                    lvl2_strand = data[3]
                    newCigar = compressCigar(updateCigar2(expandCigar(lvl1_cigar), expandCigar(lvl2_cigar), lvl1_strand, lvl2_strand))
